...
def three_point_topology_1 : topological_space (fin 3) := generate_from {{1}, {1,2}, {1,3}}

def three_point_topology_2 : topological_space (fin 3) := generate_from {{1}, {2}, {3}}

def three_point_topology_3 : topological_space (fin 3) := generate_from {{1}, {2,3}}

def three_points_topology_1' : topological_space (fin 3) :={
 is_open := λ U, U=univ ∨ U={1} ∨ U={1,2} ∨ U={1,3},
 univ_mem := by tauto,
 union := begin
   intros A hA,
   by_cases huniv: ∃ B∈ A, B=univ,
   {
     left,
     simp at huniv,
     exact sUnion_univ_of_mem_univ huniv,
   },
   by_cases h1: A ⊃ {{1,2},{1,3}},
   {
     left,
     simp,
   ...
   /-{
     ...
     by_cases h112 : A={{1},{1,2}},
     {
       right, left,
       sorry
     },
     by_cases h113 : A={{1},{1,3}},
     {
       right, left,
       sorry
     },
     by_cases h1213 : A={{1,2},{1,3}},
     {
       right, left,
       sorry
     },
     by_cases h11213 : A={{1},{1,2},{1,3}},
     {
       right, left,
       sorry
     },
